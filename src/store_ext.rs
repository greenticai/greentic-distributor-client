//! HTTP transport for store extension `.gtxpack` artifacts.
//!
//! Fetches an extension archive from the store's public REST artifact endpoint
//! (`{base}/api/v1/extensions/{name}/{version}/artifact`), verifies the
//! whole-archive `x-artifact-sha256` digest when advertised, and caches the
//! bytes on disk for offline reuse.
//!
//! This is the artifact-fetching half of unifying store-extension fetching with
//! OCI pack fetching: the transport lives here in the distributor-client so that
//! the client owns all artifact fetching. Reference resolution (parsing
//! `store://<name>@<version>` and resolving the store base URL from the
//! environment) stays in the caller (`greentic-pack`'s `ext_resolver`).
...
/// Fetch (and cache) the extension `.gtxpack` from the store artifact endpoint.
///
...
/// mismatch), caches the bytes under `cache_dir/ext-store/` keyed by BOTH
/// `sha256-<hex>.gtxpack` AND a sanitized `name@version` ref-key, and returns
/// the bytes.
///
/// # Arguments
/// * `store_base` - Store base URL (e.g. `https://store.greentic.cloud`).
/// * `name` - Extension name (the `<name>` segment of `store://<name>@<version>`).
/// * `version` - Explicit, pinned extension version.
/// * `cache_dir` - Runtime cache root; artifacts land under `cache_dir/ext-store/`.
...
pub fn fetch_store_extension(
    ...
    if offline {
        ...
        return read_cached_store_artifact(cache_dir, name, version).ok_or_else(|| {
            anyhow!(
                "offline: no cached artifact for store extension '{name}@{version}' under the cache dir; run online once to populate the cache"
            )
        });
    }

    let url = store_artifact_url(store_base, name, version);
...
mod tests {
    ...
    #[test]
    fn store_artifact_url_has_expected_shape() {
        let url = store_artifact_url("https://store.greentic.cloud", "memory-chronicle", "0.4.0");
        assert_eq!(
            url,
            "https://store.greentic.cloud/api/v1/extensions/memory-chronicle/0.4.0/artifact"
        );
    }

    #[test]
    fn store_artifact_url_trims_trailing_slash() {
        let url = store_artifact_url("https://store.greentic.cloud/", "ext", "1.2.3");
        assert_eq!(
            url,
            "https://store.greentic.cloud/api/v1/extensions/ext/1.2.3/artifact"
        );
    }
    ...
    fn cache_round_trip_by_ref_key() {
        ...
        let bytes = b"fake-gtxpack-bytes".to_vec();
        let archive_sha = hex::encode(Sha256::digest(&bytes));

        cache_store_artifact(dir.path(), "my-ext", "0.4.0", &archive_sha, &bytes)
            .expect("cache write");

        // Ref-keyed read returns the same bytes.
        let read_back = read_cached_store_artifact(dir.path(), "my-ext", "0.4.0")
            .expect("cached bytes present");
        assert_eq!(read_back, bytes);

        // The sha-keyed copy is written too.
        let sha_path =
            store_artifact_cache_dir(dir.path()).join(format!("sha256-{archive_sha}.gtxpack"));
    ...
    fn ref_cache_key_sanitizes_separators() {
        ...
        assert_eq!(key, "scope_name_1.0.0.gtxpack");
    }

    #[test]
    fn offline_miss_reports_actionable_error() {
        let dir = tempfile::tempdir().expect("tempdir");
        let err = fetch_store_extension(
        ...
        assert!(
            message.contains("offline") && message.contains("missing-ext@0.4.0"),
    ...
    fn offline_hit_serves_cached_bytes() {
        ...
        let archive_sha = hex::encode(Sha256::digest(&bytes));
        cache_store_artifact(dir.path(), "cached-ext", "0.4.0", &archive_sha, &bytes)
            .expect("cache write");

        let read_back = fetch_store_extension(
            "https://store.greentic.cloud",
            "cached-ext",
            "0.4.0",
            dir.path(),
